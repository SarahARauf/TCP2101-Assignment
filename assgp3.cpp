...
public:
    ...
    void parseCommands()
    {
        for (auto c : commands) // for each of the user's commands
        {
            ...
            if (commandType.compare("=") == 0) //= <var> <value>, = <var> <var1>
            {
                ...
                {//if the value is a variable name that is located in the runningProgram map:
                    if (varType == "integer")
                    {
                        ...
                        Variable<int> *vptr;
                        Variable<int> *vptr2;
                        vptr = static_cast<Variable<int> *>(runningProgram[var]);
                        vptr2 = static_cast<Variable<int> *>(runningProgram[value]);
                        vptr->setValue(*vptr2); //Passing vptr2

                    }
                    if (varType == "real")
                    {
                        Variable<double> *vptr;
                        Variable<double> *vptr2;
                        vptr = static_cast<Variable<double> *>(runningProgram[var]);
                        vptr2 = static_cast<Variable<double> *>(runningProgram[value]);
                        vptr->setValue(*vptr2);
                    }
                    if (varType == "bool")
                    {
                        Variable<bool> *vptr;
                        Variable<bool> *vptr2;
                        vptr = static_cast<Variable<bool> *>(runningProgram[var]);
                        vptr2 = static_cast<Variable<bool> *>(runningProgram[value]);
                        vptr->setValue(*vptr2);
                    }
            ...
            if (commandType.compare("print") == 0) //print only for int variables for now
            { 
                ...
                Variable<int> *vptr2;
                vptr2 = static_cast<Variable<int> *>(runningProgram[var]);
                cout << vptr2->getValue() << endl; 
            }
